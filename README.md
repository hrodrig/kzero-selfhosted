...
[![Version](https://img.shields.io/badge/version-0.1.11-blue)](https://github.com/hrodrig/kzero-selfhosted/releases)
...
**Tested with kzero [v0.9.0](https://github.com/hrodrig/kzero/releases/tag/v0.9.0)** — **`run.api_watchdog`**, **`pipeline.stalled`**, **`notify.require_delivery`**, graceful **SIGINT/SIGTERM**, **`kzero target --output slug`**, notify **`[ERR]`**, **`run.execution`**, **`kzero analyze`**, verify/probe/preflight. [deployment-models](https://github.com/hrodrig/kzero/blob/main/docs/deployment-models.md). **[CHANGELOG](CHANGELOG.md)** · **[kzero CHANGELOG](https://github.com/hrodrig/kzero/blob/main/CHANGELOG.md)**.
...
| **Production** `live` **down** / **up** / **reset** | [run/standalone/README.md](run/standalone/README.md) — bastion cron/systemd; **kzero**, **kubeconfig**, **`kubectl`** / **`helm`** as your YAML requires |
| **`docker run`** ( **`analyze`** / **`version`** ) | [run/docker/README.md](run/docker/README.md) — examples use **`ghcr.io/hrodrig/kzero:v0.9.0`** |
| **Large platform reset** (anonymized lab profile) | [run/examples/platform-reset/](run/examples/platform-reset/README.md) |
| **Full demo reset** (`kzero-demo`, bootstrap scripts) | [run/examples/full-reset-example/](run/examples/full-reset-example/README.md) · [validation runbook](run/docs/full-reset-validation.md) |
| **Copy-paste sample config** | [run/examples/kzero.sample.yml](run/examples/kzero.sample.yml) · reference **hooks** and **infra-probe** under [run/examples/](run/examples/README.md) |
| **CI / cron / YES-gated wrappers** | [run/docs/automation-and-pipelines.md](run/docs/automation-and-pipelines.md) |
...
From the **repository root**, with **Docker**, **kind**, **kubectl**, and **kzero** [v0.9.0+](https://github.com/hrodrig/kzero/releases/tag/v0.9.0) on **`PATH`** (or **`KZERO_BIN`**):
...
| **`run/standalone/`** | Bastion cron / systemd — [run/standalone/README.md](run/standalone/README.md) |
| **`run/docs/`** | CI/cron automation — [run/docs/automation-and-pipelines.md](run/docs/automation-and-pipelines.md) |
| **`run/docker/`** | **`docker run`** examples ([run/docker/README.md](run/docker/README.md)) |
| **`run/examples/`** | Sample YAML, **hooks**, **infra-probe** ([run/examples/README.md](run/examples/README.md)) |
