...
[![Version](https://img.shields.io/badge/version-0.1.10-blue)](https://github.com/hrodrig/kzero-selfhosted/releases)
...
**Tested with kzero [v0.8.1](https://github.com/hrodrig/kzero/releases/tag/v0.8.1)** — including **`run.api_watchdog`**, **`pipeline.stalled`** notify, notify dispatch **`[ERR]`** logs, timestamped **`[DBG|INF|WRN|ERR]`** logs, **`--log-level`**, **Slack notify** attachments and **`KZERO_NOTIFY_*`** env binding, **`run.execution`** (shell / native / auto), **per-step retry**, **`kzero analyze`**, **server-side dry-run**, **notify** / **verify** / **infra_probe** / **preflight**. See **[CHANGELOG.md](CHANGELOG.md)** and the **[kzero CHANGELOG](https://github.com/hrodrig/kzero/blob/main/CHANGELOG.md)**.
...
| **`docker run`** ( **`analyze`** / **`version`** ) | [run/docker/README.md](run/docker/README.md) — examples use **`ghcr.io/hrodrig/kzero:v0.8.1`** |
| **Full platform reset example** (truncate, PVC wipe, infra_probe) | [run/examples/full-reset-example/](run/examples/full-reset-example/README.md) · [validation runbook](run/docs/full-reset-validation.md) |
...
From the **repository root**, with **Docker**, **kind**, **kubectl**, and **kzero** [v0.8.0+](https://github.com/hrodrig/kzero/releases/tag/v0.8.0) on **`PATH`** (or **`KZERO_BIN`**; **v0.8.1** recommended):
