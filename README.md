...
[![Version](https://img.shields.io/badge/version-0.1.1-blue)](https://github.com/hrodrig/kzero-selfhosted/releases)
...
| **Disposable cluster + kzero** | [testing/kind/README.md](testing/kind/README.md) — **`make test-kind-e2e`** (tested with **kzero** [v0.2.3+](https://github.com/hrodrig/kzero/releases/tag/v0.2.3)) |
...
From the repo root, with **Docker**, **kind**, **kubectl**, and **kzero** [v0.2.3+](https://github.com/hrodrig/kzero/releases/tag/v0.2.3) installed:

```bash
make test-kind-e2e
```

This creates a **kind** cluster, builds and loads the **lab counter** image (Postgres-backed **`e2e_scratch`**), applies **Deployments** (web, counter, Postgres, Redis, RabbitMQ, lightweight **obs**), a **StatefulSet** (nginx + PVC), then runs **Phase 1** (HTML UI: **0** → three **Increment** form posts → **3**), **Phase 2** **`kzero down`** (dry-run then live) and **`kzero up`**, **Phase 3** (UI shows **0** again), and **Phase 4** (delete cluster on exit). Workloads-only smoke (**no** kzero binary): **`make test-kind-workloads`**. Details: **[testing/kind/README.md](testing/kind/README.md)**.
