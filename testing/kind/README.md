...
2. **kzero reset:** **`kzero analyze`** (normalized **`[down]`** / **`[up]`** plan on stdout; **stderr** may show **Deferred** warnings for `retry` / `worker_concurrency`), **`kzero down`** (dry-run config then live), **`kzero up`**. Postgres **pre-down** hook truncates **`public.e2e_scratch`** before scale-to-zero.
...
- **`kzero`** **v0.2.3+** installed for **`make test-kind-e2e`** ([releases](https://github.com/hrodrig/kzero/releases/tag/v0.2.3)) or set **`KZERO_BIN`** to the binary path
