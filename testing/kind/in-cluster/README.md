...
- **kzero image** with **InClusterConfig** when **`run.kubeconfig`** is empty (**`v0.7.0+`** minimum; examples pin **`v1.0.1`**, or build locally — see below)
...
| **`KZERO_IN_CLUSTER_IMAGE`** | **`ghcr.io/hrodrig/kzero:v1.0.1`** | Job container image (must match loaded image) |
