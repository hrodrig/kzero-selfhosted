...
- **kzero image** with **InClusterConfig** when **`run.kubeconfig`** is empty (**`v0.7.0+`**, or build locally — see below)
...
| **`KZERO_IN_CLUSTER_IMAGE`** | **`ghcr.io/hrodrig/kzero:v0.7.3`** | Job container image (must match loaded image) |
