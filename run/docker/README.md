...
Examples use **`v0.2.3`**; pick a tag that exists on GHCR ([kzero releases](https://github.com/hrodrig/kzero/releases)).
...
```bash
docker run --rm ghcr.io/hrodrig/kzero:v0.2.3 version
```

**Check:** **`analyze`** prints **`Config:`**, **`Schema:`**, **`Run mode:`**, indexed **`[down]`** / **`[up]`** step lists, and (when applicable) a **Deferred** block for schema fields the v1 engine does not implement yet; non-fatal warnings may appear on **stderr**. Exit code **0** on success. See [kzero SPEC — `kzero analyze`](https://github.com/hrodrig/kzero/blob/main/docs/SPECIFICATIONS.md#kzero-analyze).
